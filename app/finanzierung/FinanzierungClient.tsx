    ...
    <>
      ...
      <section style={{ background: "linear-gradient(135deg,#2C1A0E 0%,#1A0E05 100%)", padding: "120px 0 80px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "0 24px", position: "relative" }}>
          <p style={{ fontSize: "11px", fontWeight: 500, letterSpacing: "0.2em", textTransform: "uppercase", color: "#D4A017", marginBottom: "16px" }}>FINANZIERUNG & FÖRDERUNG</p>
          <h1 style={{ fontFamily: "var(--font-dm-serif, serif)", fontSize: "clamp(2.5rem, 5vw, 4.5rem)", color: "#FFFFFF", lineHeight: 1.1, marginBottom: "24px", maxWidth: "700px" }}>
            Den günstigsten{" "}<span style={{ color: "#D4A017" }}>Kredit für Sie.</span>
          </h1>
          ...
          <p style={{ fontSize: "13px", color: "rgba(212,160,23,0.7)", marginBottom: "36px" }}>Beratung überwiegend digital möglich.</p>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: "#FFFFFF", padding: "80px 0" }}>
        <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "0 24px" }}>
          <p style={{ fontSize: "11px", fontWeight: 500, letterSpacing: "0.2em", textTransform: "uppercase", color: "#D4A017", marginBottom: "12px" }}>UNSERE LEISTUNGEN</p>
          <h2 style={{ fontFamily: "var(--font-dm-serif, serif)", fontSize: "clamp(1.8rem, 3vw, 2.5rem)", color: "#2C1A0E", marginBottom: "40px" }}>Alle Finanzierungsoptionen.</h2>

          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", borderBottom: "2px solid #F0EDE8", marginBottom: "48px" }}>
            {tabs.map((t) => (
              <button key={t.id} onClick={() => setActive(t.id)}
                style={{ display: "flex", alignItems: "center", gap: "6px", padding: "12px 20px", fontSize: "13px", fontWeight: active === t.id ? 500 : 400, color: active === t.id ? "#2C1A0E" : "#7A6548", border: "none", borderBottom: active === t.id ? "2px solid #D4A017" : "2px solid transparent", background: "transparent", cursor: "pointer", marginBottom: "-2px", transition: "color 0.2s" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "60px", alignItems: "start" }} className="tab-content-grid">
            <div>
              <h3 style={{ fontFamily: "var(--font-dm-serif, serif)", fontSize: "clamp(1.5rem, 2.5vw, 2rem)", color: "#2C1A0E", marginBottom: "16px" }}>{tab.title}</h3>
              <p style={{ fontSize: "15px", color: "#7A6548", lineHeight: 1.8, fontWeight: 300, marginBottom: "28px" }}>{tab.content}</p>
              <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "36px" }}>
                {tab.bullets.map((b, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "12px", fontSize: "14px", color: "#2C1A0E", fontWeight: 300 }}>
                    <span style={{ width: "22px", height: "22px", borderRadius: "50%", backgroundColor: "#D4A017", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: "2px" }}>
      ...
      <section style={{ backgroundColor: "#F5EDE0", padding: "80px 0" }}>
        <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "0 24px" }}>
          <p style={{ fontSize: "11px", fontWeight: 500, letterSpacing: "0.2em", textTransform: "uppercase", color: "#D4A017", marginBottom: "12px" }}>FINANZIERUNGSRECHNER</p>
          <h2 style={{ fontFamily: "var(--font-dm-serif, serif)", fontSize: "clamp(1.8rem, 3vw, 2.5rem)", color: "#2C1A0E", marginBottom: "8px" }}>Wie viel kostet Ihre Finanzierung?</h2>
          <p style={{ fontSize: "14px", color: "#A89070", marginBottom: "48px" }}>Orientierungswert – für ein persönliches Angebot kontaktieren Sie uns.</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "48px", alignItems: "start" }} className="rechner-grid">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
              {([
                ...
                <div key={s.label}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
                    <label style={{ fontSize: "13px", fontWeight: 500, color: "#2C1A0E" }}>{s.label}</label>
                    <span style={{ fontSize: "13px", color: "#D4A017", fontWeight: 500 }}>{s.format(s.value)}</span>
                  </div>
                  <input type="range" min={s.min} max={s.max} step={s.step} value={s.value}
                    onChange={(e) => s.set(Number(e.target.value))} className="gold-slider"
                    style={{ width: "100%", height: "4px", background: `linear-gradient(to right, #D4A017 ${((s.value - s.min) / (s.max - s.min)) * 100}%, #E8D9C5 ${((s.value - s.min) / (s.max - s.min)) * 100}%)`, borderRadius: "4px", outline: "none" }} />
                </div>
              ))}
              {/* Laufzeit Buttons */}
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "12px" }}>
                  <label style={{ fontSize: "13px", fontWeight: 500, color: "#2C1A0E" }}>Laufzeit</label>
                  <span style={{ fontSize: "13px", color: "#D4A017", fontWeight: 500 }}>{laufzeit} Jahre</span>
                </div>
                <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  {[10, 15, 20, 25, 30].map((y) => (
                    <button key={y} onClick={() => setLaufzeit(y)}
                      style={{ flex: 1, minWidth: "52px", padding: "10px 8px", border: `1.5px solid ${laufzeit === y ? "#D4A017" : "#E8D9C5"}`, borderRadius: "8px", backgroundColor: laufzeit === y ? "#D4A017" : "#fff", color: laufzeit === y ? "#fff" : "#7A6548", fontSize: "13px", fontWeight: laufzeit === y ? 500 : 400, cursor: "pointer", transition: "all 0.2s" }}>
                      {y}J
                    </button>
                  ))}
                </div>
              ...
              {/* Nebenkosten Box */}
              <div style={{ backgroundColor: "#fff", borderRadius: "14px", padding: "20px 24px", border: "1px solid #E8D9C5" }}>
                <p style={{ fontSize: "11px", fontWeight: 500, letterSpacing: "0.12em", textTransform: "uppercase", color: "#D4A017", marginBottom: "14px" }}>KAUFNEBENKOSTEN (ca.)</p>
                {[
                  ...
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #F5EDE0" }}>
                    <span style={{ fontSize: "12px", color: "#7A6548", fontWeight: 300 }}>{label}</span>
                    <span style={{ fontSize: "12px", color: "#2C1A0E", fontWeight: 500 }}>{val.toLocaleString("de-DE")} €</span>
                  </div>
                ))}
                <div style={{ display: "flex", justifyContent: "space-between", paddingTop: "10px" }}>
                  <span style={{ fontSize: "13px", color: "#2C1A0E", fontWeight: 500 }}>Gesamt Nebenkosten</span>
                  <span style={{ fontSize: "13px", color: "#D4A017", fontWeight: 600 }}>{Math.round(kaufpreis * 0.1007).toLocaleString("de-DE")} €</span>
                </div>
              </div>
            </div>
            ...
            {/* Result Panel */}
            <div>
              <div style={{ backgroundColor: "#2C1A0E", borderRadius: "20px", padding: "36px", textAlign: "center", marginBottom: "16px" }}>
                <p style={{ fontSize: "11px", letterSpacing: "0.15em", textTransform: "uppercase", color: "#D4A017", marginBottom: "8px" }}>MONATLICHE RATE</p>
                ...
                <p style={{ fontSize: "16px", color: "#D4A017", marginBottom: "28px" }}>€ / Monat</p>
                {/* Donut Chart */}
                <div style={{ display: "flex", justifyContent: "center", marginBottom: "24px" }}>
                  {(() => {
                    ...
                    const segments = [
                      { pct: ekPct, color: "#D4A017" },
                      { pct: tilgPct, color: "#4A90A4" },
                      { pct: zinsPct, color: "#7A6548" },
                    ];
                    let offset = 0;
                    return (
                ...
                <div style={{ display: "flex", justifyContent: "center", gap: "16px", marginBottom: "24px", flexWrap: "wrap" }}>
                  {[{ color: "#D4A017", label: "Eigenkapital" }, { color: "#4A90A4", label: "Tilgung" }, { color: "#7A6548", label: "Zinsen" }].map(({ color, label }) => (
